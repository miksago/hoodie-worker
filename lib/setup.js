...
var setupHelper = function(worker, config) {
  worker.name    = config.name.replace(/^worker-/, '');
  worker.version = getWorkerVersion()

  // don't mess with passed config object
  worker.config = clone(config);
  // name is already stored in this.name
  delete worker.config.name;
...
Setup.prototype.createConfigInModulesDatabase = function() {
  this.worker.log('[Setup]\tcreating object in modules database …');
