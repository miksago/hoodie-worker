...
var setupHelper = function(worker, config) {
  ...
  .otherwise( setup.worker.handleError.bind(this) );
};
...
Setup.prototype.readGlobalConfig = function() {
  ...
  this.worker.log("[Setup]\treading global config …")
  return get('module/appconfig').then( this.setGlobalConfig.bind(this) )
};

Setup.prototype.readUserConfig = function() {
  var get = this.worker.promisify( this.worker.couch.database('modules'), 'get' )
  
  this.worker.log("[Setup]\treading user config …")
